...
namespace emma
{
	GPSConnector::GPSConnector(string host, unsigned int port)
		: m_host(host), m_port(port), m_state(DISCONNECTED), m_longitude(0), m_latitude(0), m_time(0), m_datatimeout(0), m_socket(0), _running(true)
	{
	}
	...
	{
		dtn::utils::MutexLock l1(m_socketlock);
	...
	{
		dtn::utils::MutexLock l(m_socketlock);
