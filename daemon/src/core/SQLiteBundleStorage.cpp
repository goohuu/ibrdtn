...
namespace dtn
{
	...
	//	}
		...
		void SQLiteBundleStorage::executeQuery(sqlite3_stmt *statement)
		{
			int err;
		...
		void SQLiteBundleStorage::checkFragments(set<string> &payloadFiles)
		{
			...
			sqlite3_stmt *getPayloadfiles = prepareStatement("SELECT Source, Destination, Reportto, Custodian, ProcFlags, Timestamp, Sequencenumber, Lifetime, Fragmentoffset, Appdatalength, Payloadfilename FROM "+_tables[SQL_TABLE_BUNDLE]+" WHERE Payloadfilename != NULL;");
		...
		void SQLiteBundleStorage::checkBundles(set<string> &blockFiles)
		{
			...
			for(fileList_iterator = blockIDList.begin(); fileList_iterator != blockIDList.end(); fileList_iterator++) {
				...
				executeQuery(_statements[BUNDLE_REMOVE]);
			}
		...
		dtn::data::MetaBundle SQLiteBundleStorage::get_meta(const dtn::data::BundleID &id)
		{
			...
			// get the bundle key
			const std::string ID = generate_bundle_key(id);
		...
		dtn::data::Bundle SQLiteBundleStorage::get(const dtn::data::BundleID &id)
		{
			...
			// do this while db is locked
			{
				...
				if (sqlite3_step(_statements[BUNDLE_GET_ID]) != SQLITE_ROW)
				{
					sqlite3_reset(_statements[BUNDLE_GET_ID]);
					stringstream error;
					error << "SQLiteBundleStorage: No Bundle found with BundleID: " << ID;
					IBRCOMMON_LOGGER(error) << error.str() << IBRCOMMON_LOGGER_ENDL;
					throw SQLiteQueryException(error.str());
				}
		...
		{
			std::list<dtn::data::MetaBundle> list;
			...
			try {
				...
				//where_query = "WHERE " + query.getWhere();
			} catch (const std::bad_cast&) {
				// this is not optimized query, use the generic (slow) way
			};

			int offset = 0;
			while (list.size() != cb.limit())
			{
				std::stringstream offset_ss; offset_ss << offset;
				const std::string limit_query = "LIMIT " + offset_ss.str() + ", 50";

				// prepare the statement
				sqlite3_stmt *st = prepareStatement(base_query + " " + where_query + " " + limit_query);
	...
	//	}
		...
		void SQLiteBundleStorage::store(const dtn::data::Bundle &bundle)
		{
			...
			const std::string bundle_key = generate_bundle_key(bundle);
...

			{
				...
				sqlite3_bind_text(_statements[BUNDLE_STORE], 1, bundle_key.c_str(), bundle_key.length(), SQLITE_TRANSIENT);
	...
	//				sqlite3_bind_int(_statements[BLOCK_STORE], 2, (int)((*it)->getType()));
		...
		void SQLiteBundleStorage::raiseEvent(const Event *evt)
		{
			...
			try {
				...
				{
					ibrcommon::MutexLock lock(timeeventConditional);
		...
		void SQLiteBundleStorage::deleteexpired()
		{
			...
			list<data::BundleID>::iterator it;
			{
				...
				executeQuery(_statements[BUNDLE_DELETE_TTL]);
			}
		...
		void SQLiteBundleStorage::get_blocks(dtn::data::Bundle &bundle, const std::string &ID)
		{
			int err = 0;
			string file;
			std::list<ibrcommon::File> files;
			...
			if (sqlite3_bind_text(_statements[BLOCK_GET_ID], 1, ID.c_str(), ID.length(), SQLITE_TRANSIENT) != SQLITE_OK)
			{
			...
			if (err == SQLITE_DONE)
			{
				if (files.size() == 0)
				{
					IBRCOMMON_LOGGER(error) << "get_blocks: no Blocks found for: " << ID << IBRCOMMON_LOGGER_ENDL;
					throw SQLiteQueryException("no blocks found");
				}
			}
			else
			{
				IBRCOMMON_LOGGER(error) << "get_blocks() failure: "<< err << " " << sqlite3_errmsg(database) << IBRCOMMON_LOGGER_ENDL;
		...
		const std::string SQLiteBundleStorage::generate_bundle_key(const dtn::data::BundleID &id) const
		{
			return id.toString();
		}
		...
		{
			ibrcommon::MutexLock lock(time_change);
