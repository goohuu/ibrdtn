...
namespace dtn
{
	namespace daemon
	{
		...
		RSA * SecurityManager::loadKey(std::map <dtn::data::EID, RSA* >& map, const dtn::data::EID& eid, SecurityBlock::BLOCK_TYPES bt)
		{
			...
			if (it == map.end())
			{
				std::string key = Configuration::getInstance().getPublicKey(eid.getNodeEID(), bt);
				if (key.size() > 0)
				{
		...
		RSA * SecurityManager::loadKey_public(RSA ** rsa, SecurityBlock::BLOCK_TYPES bt)
		{
			if (*rsa == NULL)
			{
				std::pair<std::string, std::string> keys = Configuration::getInstance().getPrivateAndPublicKey(bt);
		...
		std::string SecurityManager::loadKey(std::map<dtn::data::EID, std::string>& map, const dtn::data::EID& node, SecurityBlock::BLOCK_TYPES bt)
		{
			...
			if (it == map.end())
			{
				std::string keypath = Configuration::getInstance().getPublicKey(node.getNodeEID(), bt);
				if (keypath.size() == 0)
					return key;
		...
		std::list< KeyBlock > SecurityManager::getListOfNeededKeys(const dtn::data::Bundle& bundle) const
		{
			...
			// asymmetric keys
			std::list<Configuration::SecurityRule::RuleToken> rules = getRule(bundle._destination.getNodeEID()).getRules();
			for (std::list<Configuration::SecurityRule::RuleToken>::iterator it = rules.begin(); it != rules.end(); it++)
			{
...
//			_bundle_with_neighbors[BundleID(bundle)].insert(eid);
		...
		{
			std::list<dtn::daemon::Configuration::SecurityRule> rules = Configuration::getInstance().getSecurityRules();
		...
		Configuration::SecurityRule SecurityManager::getRule(const dtn::data::EID& eid) const
		{
			...
			Configuration::SecurityRule the_rule;
